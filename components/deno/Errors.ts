...
import { TToken, Token } from "./parser/Scanner.ts";
...
export class SyntaxErrorException extends Error {
    ...
    toString(): string {
        return `Syntax Error: Expected ${commaSeparated(this.expected.map(t => ttokens.get(t)))} but found ${ttokens.get(this.found[0])} at ${locationToString(this.src, this.found[1])}`;
    }
}
