...
class InferTest {
    ...
    fun inferConsPattern() {
        ...
        assertInferPatternWithEnv(origEnv, PDataPattern(null, "Nil", emptyList()), emptyList(), "List V1", origEnv)
        assertInferPatternWithEnv(
            origEnv,
            PDataPattern(null, "Cons", listOf(PVarPattern("x"), PVarPattern("xs"))),
    ...
    fun inferVarPattern() {
        assertInferPattern(
            ...
            emptyList(),
            "V1",
            emptyTypeEnv.extend("x", Scheme(emptySet(), TVar("V1")))
        )
    }
