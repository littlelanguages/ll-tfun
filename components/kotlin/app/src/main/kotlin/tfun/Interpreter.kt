...
private fun executeElement(
    ...
    when (e) {
        ...
        is ImportStatement -> TODO()
        is TypeAliasDeclaration -> TODO()
    }
...
private fun evaluate(ast: Expression, env: RuntimeEnv): Value? =
    when (ast) {
        ...
        is LamExpression ->
            { x: Any? -> evaluate(ast.e, env + Pair(ast.n.name, x)) }
        ...
        is FatBarExpression -> TODO()
            ...
            println("Unknown expression: $ast")
            null
        }
    }
            ...
            if (value.size > 1)
                "$name ${
                    value.drop(1).joinToString(" ") {
                        if (it is Array<*> && it.size > 1) "(${valueToString(it)})" else valueToString(it)
                    }
                }"
            else
                name
