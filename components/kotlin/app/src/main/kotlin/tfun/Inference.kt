...
private class Inference(val constraints: Constraints, val pump: Pump) {
    fun infer(typeEnv: TypeEnv, e: Expression): InferenceResult =
        when (e) {
            ...
            is LamExpression -> {
                val tv = pump.next()
                val t = infer(typeEnv + Pair(e.n.name, Scheme(setOf(), tv)), e.e).type
            ...
            is LetRecExpression -> {
                ...
                val declarationType =
                    fix(
                        interimTypeEnv,
                        LamExpression(Parameter("_bob", null), LTupleExpression(e.decls.map { it.e }), null),
                        constraints
                    )
